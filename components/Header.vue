...
import pokeballIcon from 'assets/images/general/pokeball-icon.png';

const query = ref('');

async function search() {
  await fetch(`https://pokeapi.co/api/v2/pokemon/${query.value}`)
      .then(response => response.json())
      .then(data => {
        navigateTo(`/pokemon/${data.id}`)
      })
}
</script>

<template>
  <nav class="w-full h-24 border-gray-200 bg-gray-900 fixed z-10">
    <div class="max-w-screen-xl h-full flex flex-wrap items-center justify-between mx-auto p-4">
      <a href="/" class="flex items-center space-x-3 rtl:space-x-reverse">
        <img :src="pokeballIcon" class="h-8" alt="Flowbite Logo"/>
